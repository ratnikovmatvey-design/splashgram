...
void Step::paintCover(QPainter &p, int top) {
	...
	const auto splashgramIcon = Ui::PixmapFromImage(AyuAssets::currentAppLogo());
	QIcon(splashgramIcon).paint(&p, QRect(width() / 2 - splashgramIcon.width() / 2, planeTop - 16, splashgramIcon.width(), st::introCoverIcon.height()));
}
